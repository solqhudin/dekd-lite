            ...
            <div class="px-6 md:px-8 py-8">
                ...
                <form method="POST" action="{{ route('admin.announcements.store') }}" enctype="multipart/form-data" class="grid gap-5">
                    ...
                    <div>
                        ...
                        <input name="title" value="{{ old('title') }}" class="mt-1 w-full rounded-lg border-gray-300">
                    </div>

                    <div class="grid sm:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Slug (ไม่บังคับ)</label>
                            <input name="slug" value="{{ old('slug') }}" class="mt-1 w-full rounded-lg border-gray-300" placeholder="ปล่อยว่างให้ระบบสร้างให้ก็ได้">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">หมวด</label>
                            <input name="category" value="{{ old('category') }}" class="mt-1 w-full rounded-lg border-gray-300" placeholder="เช่น ข่าว, กิจกรรม">
                        </div>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700">คำโปรย (Excerpt)</label>
                        <textarea name="excerpt" rows="2" class="mt-1 w-full rounded-lg border-gray-300">{{ old('excerpt') }}</textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700">เนื้อหา (Body) รองรับ HTML</label>
                        <textarea name="body" rows="8" class="mt-1 w-full rounded-lg border-gray-300">{{ old('body') }}</textarea>
                    </div>
                    ...
                    <div class="grid sm:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="published_at" value="{{ old('published_at') }}" class="mt-1 w-full rounded-lg border-gray-300">
                        </div>
                    </div>
