...
<div class="max-w-6xl mx-auto px-4 py-10">
  <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-6">
    ...
    <div class="flex items-center gap-2 sm:gap-3">
      {{-- ปุ่มสร้างประกาศ (เฉพาะแอดมิน/ผู้มีสิทธิ์) --}}
      @auth
        @can('manage-announcements')
          <a href="{{ route('admin.announcements.create') }}"
             class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-brand-700 text-white text-sm font-semibold hover:brightness-110 shadow-sm">
            + สร้างประกาศ
          </a>
        @endcan
      @endauth
      ...
      @foreach($announcements as $a)
        <a href="{{ route('announcements.show', $a->slug) }}"
          ...
          <p class="mt-1 text-sm text-gray-600 line-clamp-2">
            {{ $a->excerpt ?: \Illuminate\Support\Str::limit(strip_tags($a->body), 120) }}
          </p>
