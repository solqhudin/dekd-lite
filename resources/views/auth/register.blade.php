...
@section('content')
    <div class="flex items-center justify-center px-4 py-10">
        <div
            class="w-full max-w-6xl rounded-[24px] overflow-hidden bg-white shadow-card ring-1 ring-black/5 grid lg:grid-cols-2">
            ...
            <section class="relative min-h-[460px] p-10 flex items-center bg-brand-700">
                ...
                <div class="pointer-events-none absolute bottom-10 right-10 h-52 w-52 rounded-full bg-white/10 blur-2xl">
                </div>

                <div class="relative z-10 text-white w-full">
                    <div class="flex flex-col items-center gap-3 mb-6 text-center">
                        <img src="{{ asset('images/logoWhite.png') }}" alt="Engenius Group"
                            class="h-28 md:h-32 lg:h-36 w-auto object-contain block mx-auto" />
                        {{-- <span class="text-base md:text-lg tracking-wide text-white/90">Engenius Group</span> --}}
                    </div>

                    <h1 class="text-center text-4xl md:text-5xl font-extrabold leading-tight drop-shadow-sm">
                        สร้างบัญชีใหม่
                    </h1>
                    <p class="mt-3 max-w-md mx-auto text-center text-white/90">
                        ลงทะเบียนเพื่อเริ่มใช้งานพอร์ทัล จัดการเครือข่าย คลาวด์ และ IoT ได้สะดวก ปลอดภัย และมีประสิทธิภาพ
                    </p>

                    <div class="mt-8 flex flex-wrap justify-center items-center gap-3 text-xs md:text-sm text-white/80">
                        <span class="inline-flex items-center gap-1">
                            <span class="h-1.5 w-1.5 rounded-full bg-white/80"></span> Single Sign-On
                        </span>
                        <span class="inline-flex items-center gap-1">
                            <span class="h-1.5 w-1.5 rounded-full bg-white/80"></span> Security First
                        </span>
                        <span class="inline-flex items-center gap-1">
                            <span class="h-1.5 w-1.5 rounded-full bg-white/80"></span> 24/7 Access
                        </span>
                    </div>
                </div>
            </section>
                ...
                <form method="POST" action="{{ route('register') }}" class="mt-6 grid gap-5">
                    ...
                    <label class="inline-flex items-start gap-2 text-sm text-gray-700">
                        <input type="checkbox" required
                            class="mt-[3px] rounded border-gray-300 text-brand-700 focus:ring-brand-700">
                        <span>
    ...
    <script>
        ...
        (function() {
            ...
            pw.addEventListener('input', () => {
                ...
                label.className = 'text-xs ' + (score <= 1 ? 'text-red-600' : score <= 3 ? 'text-amber-600' :
                    'text-emerald-600');
            });
        })();
    </script>
